...
import { State, createDonation, updateDonation } from '@/app/lib/actions';
import { Donation } from '@/app/lib/definitions';

export default function NerchaDonationEditForm({
  nerchaId,
  donation,
}: {
  nerchaId: string;
  ...
  const donationId = donation ? donation.id : '';
  const updateDontationWithId = updateDonation.bind(null, donationId);
  const [state, dispatch] = useFormState(updateDontationWithId, initialState);
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <select
              ...
              name="clusterId"
              className="peer block w-full rounded-md border border-gray-200 py-2 pl-2 text-sm outline-2 placeholder:text-gray-500"
              defaultValue={donation?.cluster_id}
            ...
            <div className="flex gap-4">
              <div className="flex items-center">
                <input
                  ...
                  defaultChecked={donation?.status === 'pending'}
                />
                <label
                  htmlFor="pending"
              ...
              <div className="flex items-center">
                <input
                  ...
                  type="radio"
                  value="paid"
                  defaultChecked={donation?.status === 'paid'}
                  className="h-4 w-4 cursor-pointer border-gray-300 bg-gray-100 text-gray-600 focus:ring-2"
                />
                <label
                  htmlFor="paid"
